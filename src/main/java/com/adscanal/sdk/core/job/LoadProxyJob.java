    ...
    public static void rebuildCustomer(LiveOffer offer) {
        ...
        if (oldclicks == null || Math.abs(clicks - oldclicks) > 20000) {
            ...
            for (int i = 0; i < coresize; i++) {
                SdkConf.OFFER_SCHED_NEW.get(offer.getUid() + "").scheduleAtFixedRate(new OfferTask(offer, offer.getCountry().toUpperCase() + offer.getOsName().toLowerCase(), offer.getCountry().toUpperCase(), offer.getOsName().toLowerCase()), 10 * 1000, period*40, TimeUnit.MILLISECONDS);
            }
